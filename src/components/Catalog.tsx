    ...
    <main className="catalog">
      <header className="catalog-header">
        <a className="brand-lockup" href={import.meta.env.BASE_URL} aria-label="Каталог курса">
          ...
          <span><strong>МДК.06.02</strong><small>Инженерно-техническая поддержка сопровождения информационных систем</small></span>
        </a>
      ...
      <section className="catalog-hero">
        <div className="hero-copy">
          ...
          <h1>Поддержка, сопровождение <span>и восстановление систем</span></h1>
          <p className="hero-lead">От обращения пользователя и измеримого SLA — к восстановленной услуге, понятному регламенту и предупреждению повторных отказов.</p>
        ...
        <div className="hero-mascot">
          ...
          <img src={asset('brand/mascot/okfks-rhino-catalog.png')} alt="Носорог — специалист инженерно-технической поддержки" />
        </div>
      </section>
        ...
        {filtered.map((topic) => (
          <article className={`topic-card semester-${topic.semester}`} key={topic.id}>
            ...
            <div className="topic-card-footer">
              <span>Тема {topics.filter((item) => item.semester === topic.semester).findIndex((item) => item.id === topic.id) + 1} из {topics.filter((item) => item.semester === topic.semester).length}</span>
